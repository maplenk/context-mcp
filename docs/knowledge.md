...
│   └── mcp/
│       ├── server.go               — mcp-golang SDK server over stdio + deferred tool registration (PendingSDKTool, ActivateTool)
│       ├── tools.go                — 20 CLI tools, profile-gated SDK tools (core=7, extended=14, full=17, minimal=3+discover)
│       ├── tools_test.go           — 35+ tool tests
│       ├── discovery.go            — Task bundles, discover_tools, execute_tool (minimal profile entry point)
│       ├── discovery_test.go       — 12 discovery tests + schema footprint assertion
│       ├── compact_test.go         — Compact output mode tests (7 tools)
│       ├── output.go               — OutputStore + retrieve_output (tiered output sandbox: 8KB warn, 16KB sandbox)
│       └── output_test.go          — 11 output sandbox tests
├── tests/
...
- `NodeType` uint8: Function(1), Class(2), Struct(3), Method(4), Interface(5), File(6), Route(7)
- `EdgeType` uint8: Calls(1), Imports(2), Implements(3), Instantiates(4), Defines(5), DefinesMethod(6), Inherits(7), Handles(8)
...
- **Migration v4**: Index on `git_commits(author_time)` for efficient commit ordering
...
- Per-file cap: configurable via `max_per_file` parameter (default 1), non-core files (penalty ≤ NonCoreThreshold 0.5) capped at 1
...
### Phase 14: Dual-Mode Discovery + Output Cost Reduction (Commits 53-56)

| # | Hash | Description | Agent | Status |
|---|------|-------------|-------|--------|
| 53 | `3c63150` | Compact output mode for 7 tools (25-63% reduction) | Opus (worktree) | Done |
| 54 | `8b0a84e` | Minimal profile + bundle-driven discovery (3 tools, 1244 bytes schema) | Opus (worktree) | Done |
| 55 | `1d6576d` | Output sandbox (8KB warn, 16KB auto-sandbox, retrieve_output pagination) | Opus (worktree) | Done |
| 56 | `6986a7f` | DA review fixes: 2 CRITICAL, 3 HIGH, 2 LOW | Opus (worktree) | Done |

**Key changes:**
- **Compact output mode** (`compact: true`): Added to context, impact, understand, explore, detect_changes, architecture_summary, assemble_context. Strips Reason, WhyNow, NextTool, NextArgs — keeps IDs, scores, line spans. 25-63% size reduction.
- **Minimal profile** (`--profile minimal`): 3 SDK tools (~1716 bytes): discover_tools, execute_tool, health. Agents call discover_tools to activate curated bundles on demand.
- **Deferred registration**: All 17 registerXxxTool functions refactored — tools always built, SDK-registered only when profile matches, otherwise stored as pending for dynamic activation via ActivateTool.
- **Task bundles** (discovery.go): inspection (4 tools), change_analysis (3), architecture (3), assembly (4). Keyword matching with bundle scoring, fallback to per-tool matching.
- **execute_tool proxy**: Fallback for clients that don't handle tools/listChanged. Wraps non-activated tools with proxy_warning.
...
- `internal/mcp` — 72+ tests (CLI handlers, SDK protocol, initialize, tools/list, tools/call, all 20 tools, concurrent registration, data race fix, 5 core blueprint tool handler tests, InspectableResponse context test, architecture summary inspectable test, **7 compact mode tests**, **12 discovery tests** (bundle matching, direct activation, cap, idempotency, proxy, schema footprint, deferred registration), **11 output sandbox tests** (store/retrieve, pagination, TTL eviction, tiered thresholds, integration))
...
## Competitive Landscape (as of 2026-04-07)

### Top Competitors by Stars

| Project | Stars | Key Differentiator |
|---------|-------|--------------------|
| Serena | 22.5K | LSP-backed, 40+ languages, refactoring |
| Context Mode | 6.7K | Context sandboxing via hooks, 98% output reduction |
| Claude Context (Zilliz) | 5.9K | BM25+vector hybrid, requires OpenAI+Milvus |
| jCodeMunch-MCP | 1.5K | Token-budgeted assembly, PageRank, blast radius |
| Codebase-Memory-MCP (DeusData) | 1.3K | Knowledge graph with Cypher, 66 languages |
| CocoIndex Code | 1.3K | AST+embeddings, zero-config |

### context-mcp Unique Strengths

No single competitor matches all of these:

1. **PPR + BM25 + Betweenness Centrality + Semantic Similarity** ranking (multi-signal fusion)
2. **Single binary, zero cloud dependencies** (no OpenAI key, no Milvus, no external services required)
3. **17 MCP tools + 5 prompts + 4 resources** (broadest local tool surface)
4. **Sub-120ms query latency** on 18K+ node codebases
5. **Token-budgeted context assembly** (fit context into token limits)
6. **Blast radius analysis** with graph-aware ranking

---

## Token Reduction Strategies in the Ecosystem

Key patterns emerging in 2026:

1. **Code Execution Mode** (Cloudflare/Anthropic) -- 98.7% reduction. Expose `search()` + `execute()` instead of N tool definitions. This is the biggest strategic shift: it makes traditional tool-definition-heavy MCPs less relevant.
2. **Dynamic Toolsets** (Speakeasy) -- 96.7% reduction. Meta-tools: `search_tools`, `describe_tools`, `execute_tool`.
3. **Context Sandboxing** (Context Mode) -- 98% reduction. Intercept outputs, store in FTS5, return metadata only.
4. **On-Demand Tool Loading** (Anthropic Tool Search) -- 85% reduction. `defer_loading: true` in tool definitions.
5. **Signature-Only Retrieval** -- 96% reduction. Return function signatures, not full file contents.

---

## Gaps and Opportunities

### Missing from context-mcp

1. **Language coverage** -- Currently supports Go, JS/TS, PHP. Competitors like Serena (40+ via LSP) and DeusData (66 via tree-sitter) support far more languages.
2. **Code Mode compatibility** -- No `search()` + `execute()` pattern for the Cloudflare/Anthropic code-execution future.
3. **Session continuity** -- No integration with Claude Code compaction hooks to preserve context across resets.
4. **Git co-change signals** -- "Files that change together" as a ranking signal (TeaRAGs/SoulForge are starting this).
5. **Multi-repo support** -- Only paid solutions (Qodo, Nia) support this currently.
6. **Reproducible benchmarks** -- Need to run against the same repos competitors use (express, fastapi, gin) for fair comparison.
7. **Architecture rule enforcement** -- Only jCodeMunch has layer violation detection.
8. **Documentation indexing** -- Most competitors ignore docs; Nia indexes docs alongside code.

---

## Benchmark Data (2026-04-03)

- **context-mcp**: 12.7% hit rate on qbapi benchmark
- **Claude-ref (baseline)**: 27.1% hit rate on the same benchmark
- **Cross-file context** is the biggest gap between context-mcp and the reference
- **Token reduction**: ~90% (9,600 tokens down to ~940 tokens for typical queries)
- Competitive benchmark claims: jCodeMunch claims 95%, DeusData claims 99.2% (but measured against worst-case baselines, not head-to-head)
...
- The MCP Tax: https://www.mmntm.net/articles/mcp-context-tax
